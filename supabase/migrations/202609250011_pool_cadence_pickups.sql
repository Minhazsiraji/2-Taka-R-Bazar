-- Weekly/monthly pool cadence plus admin-selected pickup options per pool.
alter table public.pools add column if not exists cadence text not null default 'weekly';
alter table public.pools drop constraint if exists pools_cadence_check;
alter table public.pools add constraint pools_cadence_check check (cadence in ('weekly','monthly'));

create table if not exists public.pool_pickup_points (
  pool_id uuid not null references public.pools(id) on delete cascade,
  pickup_point_id uuid not null references public.pickup_points(id) on delete cascade,
  created_at timestamptz not null default now(),
  primary key (pool_id, pickup_point_id)
);

insert into public.pool_pickup_points(pool_id,pickup_point_id)
select p.id,pp.id from public.pools p
join public.pickup_points pp on pp.community_id=p.community_id and pp.active=true
on conflict do nothing;

alter table public.pool_pickup_points enable row level security;
grant select,insert,delete on public.pool_pickup_points to authenticated;

drop policy if exists pool_pickup_points_read on public.pool_pickup_points;
create policy pool_pickup_points_read on public.pool_pickup_points for select to authenticated using (
  private.has_role((select auth.uid()),'admin') or exists (
    select 1 from public.pools po join public.profiles pr on pr.id=(select auth.uid())
    where po.id=pool_id and po.community_id=pr.community_id
  )
);

drop policy if exists pool_pickup_points_admin_write on public.pool_pickup_points;
create policy pool_pickup_points_admin_write on public.pool_pickup_points for all to authenticated
...
declare
  ...
  v_pool public.pools%rowtype; v_profile public.profiles%rowtype; v_order_id uuid;
  v_order_status text; v_order_item_id uuid; v_pickup uuid; v_amount numeric(12,2); v_saving numeric(12,2);
begin
  ...
  if found then
    ...
    if v_pickup is null or not exists (
      ...
      join public.pool_pickup_points ppp on ppp.pickup_point_id=pp.id
  ...
  values(v_user,'order_item_confirmed','order',v_order_id,jsonb_build_object('commitment_id',v_c.id,'order_item_id',v_order_item_id,'amount',v_amount,'pickup_point_id',v_pickup));
  return v_order_id;
end; $$;
