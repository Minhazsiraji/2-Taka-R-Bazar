    ...
    <div className="min-h-screen overflow-x-hidden bg-slate-50 text-slate-950">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white md:hidden">
        <div className="grid grid-cols-5 text-center text-[11px]">
          {customerNav.map(([href, label]) => <Link key={href} href={href} className="min-h-14 px-1 py-3 font-semibold text-slate-700">{label}</Link>)}
        </div>
      </nav>
      <footer className="hidden border-t bg-white md:block">
        <div className="mx-auto flex max-w-6xl flex-wrap gap-4 px-4 py-4 text-sm">{customerNav.map(([href, label]) => <Link key={href} href={href}>{label}</Link>)}</div>
      </footer>
    </div>
  )
}
