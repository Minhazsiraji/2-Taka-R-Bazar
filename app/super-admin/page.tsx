...
export default async function SuperAdminDashboard() {
  ...
  return <SuperAdminShell><div className="grid min-w-0 gap-4 sm:gap-5">
    <section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Owner command center</p><h1 className="mt-1 text-2xl font-black sm:text-3xl">Business intelligence</h1><p className="mt-1 text-sm text-slate-500">Live database view for {profile?.full_name ?? 'Super Admin'} · refreshed {dateTime(now.toISOString())}</p></div><div className="grid w-full grid-cols-1 gap-2 sm:w-auto sm:grid-cols-2"><Link href="/admin/pools" className="btn-primary w-full sm:w-auto">Create / manage pool</Link><Link href="/admin" className="btn-secondary w-full sm:w-auto">Operations</Link></div></section>
    ...
    <section className="grid gap-4 2xl:grid-cols-2">
      ...
      <div><div className="mb-3"><div className="card-title">Governance</div><h2 className="section-title">Recent audit activity</h2></div><div className="card divide-y divide-slate-100 p-0">{audit.length?audit.slice(0,10).map(row=><div key={row.id} className="p-3"><div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-3"><div><b className="text-sm">{row.event_type}</b><p className="muted">{row.entity_type} · {row.entity_id ?? '—'}</p></div><span className="text-xs text-slate-400">{dateTime(row.created_at)}</span></div></div>):<div className="p-4 muted">No audit events yet.</div>}<Link href="/super-admin/audit" className="block p-3 text-sm font-black">Open full audit trail →</Link></div></div>
    </section>

    <section><div className="mb-3"><div className="card-title">Data coverage</div><h2 className="section-title">Live record snapshot</h2></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-8">{snapshot.map(([label,value])=><div className="rounded-xl border border-slate-200 bg-white p-3" key={String(label)}><div className="text-xs font-bold text-slate-500">{label}</div><div className="mt-1 text-xl font-black">{value}</div></div>)}</div></section>

    <section className="rounded-2xl bg-slate-950 p-5 text-white"><div className="card-title text-white/50">Quick decisions</div><div className="mt-3 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">{[['New pool','/admin/pools'],['Update market price','/admin/market-prices'],['Review suppliers','/admin/suppliers'],['Check commitments','/admin/commitments'],['Orders','/admin/orders'],['Pickup points','/admin/pickup-points'],['Issues','/admin/issues'],['Payments','/super-admin/payments']].map(([label,href])=><Link key={href} href={href} className="rounded-xl bg-white px-3 py-2 text-center text-sm font-black text-black">{label}</Link>)}</div></section>
  </div></SuperAdminShell>
}
