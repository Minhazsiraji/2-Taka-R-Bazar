...
export default async function VerifyOtpPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10"><section className="card w-full max-w-md p-6 sm:p-8">
    <Link href="/" className="block"><img src="/brand-logo.webp" alt="2-TAKA-R-BAZAR" className="mx-auto w-52" /></Link>
    <div className="mb-6 mt-5 text-center"><h1 className="text-3xl font-black">Verify your mobile</h1><p className="muted mt-2">Enter the 6-digit OTP sent to <b className="text-slate-800">{masked}</b>.</p></div>
    ...
    <p className="mt-5 text-center text-sm text-slate-600">Wrong number? <Link className="font-bold text-black underline underline-offset-4" href={mode === 'signup' ? '/signup' : '/login'}>Change mobile number</Link></p>
  </section></main>
}
